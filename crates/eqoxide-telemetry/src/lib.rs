...
//!
//! ## Crate boundary (#544 Step 2k)
//!
//! Extracted from `src/eq_net/packet_telemetry.rs` into its own crate, sitting directly above
...
//! `eqoxide_protocol::protocol`. The app crate re-exports this crate from `eqoxide::eq_net::packet_telemetry`
//! (`pub use eqoxide_telemetry::*;`) so every existing `crate::eq_net::packet_telemetry::…` call
//! site (`src/main.rs`, `src/eq_net/transport.rs`, `src/http/observe.rs`) keeps resolving unchanged.
//! This is a behavior-preserving pure move: the capture hook, ring, and decoded records are
//! byte-identical.
...
///
/// The arms are the constant NAMES from [`eqoxide_protocol::protocol`], compared by value — so this
/// table is compile-time-linked to the audited opcode constants (`protocol/mod.rs`) and cannot
/// silently drift from them. First match wins for the handful of values shared across login/world
/// revisions.
pub fn opcode_name(op: u16) -> &'static str {
    use eqoxide_protocol::protocol as p;
    macro_rules! names {
        ($($name:ident),* $(,)?) => {
...
/// process-global, so tests in DIFFERENT crates (this crate's own unit tests AND the app crate's HTTP
/// endpoint test in `eqoxide::http::observe`, #544 Step 2k) that enable/clear/read capture must hold
/// this SAME lock — never a second, independent lock (that was the original #532 hazard) — or they
/// clobber each other under cargo's parallel test runner. Poison-tolerant: a panicking test still
/// releases a usable guard.
///
/// Gated on `test-fixtures` (not bare `#[cfg(test)]`) and `pub` — not `pub(crate)` — so it crosses
/// the crate boundary to the app crate's test build, the same seam already used for
/// `eqoxide-core`'s `RegionMap` fixtures and `eqoxide-command`'s `goto_target` (see this crate's
/// `Cargo.toml`). A bare `#[cfg(test)]` item is invisible outside `eqoxide-telemetry`'s own `cargo
/// test` — `cfg(test)` is per-crate, so it would NOT exist in the rlib the app crate links against
/// even when the app crate's own tests are running. Off in normal (non-test, non-fixture) builds —
/// the release binary never contains it.
///
/// ONE THING THIS LOCK CANNOT DO (#541): it only serializes tests that explicitly take it. Dozens of
/// tests elsewhere in the app crate (`eq_net::action_loop`, `eq_net::gameplay`, `eq_net::transport`)
...
pub fn test_capture_lock() -> std::sync::MutexGuard<'static, ()> {
