...
async fn post_target(
    ...
    let known = is_self || s.world.entity_ids().values().any(|&v| v == id);
    if !known {
...
mod tests {
    ...
    async fn target_unknown_spawn_id_is_404_and_queues_nothing() {
        ...
        let state = empty_state();
        state.world.entity_ids_mut().insert_for_test("a rat000".into(), 7);
        let command = state.command.clone();
        let app = router().with_state(state);
    ...
    async fn target_name_partial_only_is_flagged_fuzzy() {
        let state = empty_state();
        state.world.entity_ids_mut().insert_for_test("Astaed_Wemor007".into(), 77);
        let app = router().with_state(state);
        let req = Request::post("/target/name")
            .header("content-type", "application/json")
