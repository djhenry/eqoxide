...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.len() != 1 && args.len() != 7 {
        return Err("usage: eqg_collision_probe COLLISION.glb [FROM_X FROM_Y FROM_Z TO_X TO_Y TO_Z]".into());
    }
    let segment = if args.len() == 7 {
        ...
        if !values.iter().all(|v| v.is_finite() && v.abs() <= 1_000_000.0) {
            return Err("segment coordinates must be finite and within one million source units".into());
        }
