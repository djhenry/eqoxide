...
impl RegionMap {
    ...
    /// Test-only: a zone-line (`DRNTP`) region carrying `index`, bounded to the XY box
    /// `[n0,n1] × [e0,e1]` (north × east) AND the z-slab `[zbot,ztop]` — everywhere else reads as
    ...
    pub fn zone_line_box(n0: f32, n1: f32, e0: f32, e1: f32, zbot: f32, ztop: f32, index: i32) -> RegionMap {
        ...
        // leaf_at swaps to (lx,ly,lz) = (north, east, z), so normal[0]=north, normal[1]=east, normal[2]=z.
        // Six axis planes; the leaf inside ALL of them (node 8) is the zone line, dry (node 7) otherwise.
        RegionMap { nodes: vec![
            BspNode { normal: [ 1.0, 0.0, 0.0], split: -n0,  special: 0, left: 2, right: 7, zone_line_index: 0 }, // north >= n0
            BspNode { normal: [-1.0, 0.0, 0.0], split:  n1,  special: 0, left: 3, right: 7, zone_line_index: 0 }, // north <= n1
            BspNode { normal: [ 0.0, 1.0, 0.0], split: -e0,  special: 0, left: 4, right: 7, zone_line_index: 0 }, // east  >= e0
            BspNode { normal: [ 0.0,-1.0, 0.0], split:  e1,  special: 0, left: 5, right: 7, zone_line_index: 0 }, // east  <= e1
            BspNode { normal: [ 0.0, 0.0, 1.0], split: -zbot,special: 0, left: 6, right: 7, zone_line_index: 0 }, // z     >= zbot
            BspNode { normal: [ 0.0, 0.0,-1.0], split:  ztop,special: 0, left: 8, right: 7, zone_line_index: 0 }, // z     <= ztop
            dry, // 7
            zl,  // 8
        ]}
    }
