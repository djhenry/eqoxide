...
* **The memory/branching math is the trap the brief names.** Everfrost's whole-zone close is
  **1.12M nodes** on the current surface graph, and `MAX_NODES` = 8M is the absolute backstop
  (cited by SOURCE TEXT, not line number: `pub const MAX_NODES: usize = 8_000_000;` in
  `collision.rs`, and its doc comment. The line numbers this citation originally carried —
  `:2082-2083`, `:74` — are stale, and the reason is worth keeping: that doc has been rewritten
  three times since, and `:74` has landed on a different claim each time, so the line number was
  never a stable handle even while the constant was. Cite the text. **The 1.12M is the
  NO-REGION-MAP figure** — it was measured on a `Collision` with no region data attached, a
  ...
  voxelization of a 6400×6400u zone with a modest 200u vertical band is 3200·3200·100 =
  **1.0×10⁹ voxels** [derived] — ~10³× everfrost's surface graph (~200× butcher's) and 128× the
  backstop, with a 26-way branching factor on top. Water, by contrast, is small and
