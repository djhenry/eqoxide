...
- Clearing happens on the same ~10 s standing probe, so `zone_cross_stopped` can remain non-null for
  up to one cooldown after you have already stepped off. *(Reasoned from the code path, not
  measured live.)*
