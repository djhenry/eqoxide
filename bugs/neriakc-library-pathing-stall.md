...
**Summary:** `/v1/navigate/goto` toward the Third Gate library stalls: the player walks ~35
...
the library NPC Lokar To`Biath — so any quest there is unreachable by `/v1/navigate/goto`).
...
1. Mordeth (Dark Elf SK) in `neriakc`, standing near the Lodge at approx
   `(-1282, 1249, -80)`.
2. `POST /v1/navigate/goto {"name":"Lokar_To`Biath000"}` (Lokar is in the library at approx
   `(-1246, 894, -66.6)`).
3. Poll `GET /v1/observe/debug` for ~25s.
...
dropped from -80 to -87 (stepped down into the passage). A `/v1/observe/frame` shows the
character wedged against stone walls in a narrow corridor/corner. `/v1/navigate/warp` to
...
- `/v1/navigate/warp` (collision-bypassing teleport) reaches the target, so this is a
