# /v1/observe/inventory API does not expose currency amounts

**Summary:** `GET /v1/observe/inventory` returns carried items only; there is no way to read
...
**Zone / area:** HTTP API (`/v1/observe/inventory`, and the player-state in `/v1/observe/debug`).

## Steps to reproduce
1. Log in any character (e.g. Mordeth on port 8766).
2. `GET /v1/observe/inventory` → `{count, items:[...]}` — items only, no money field.
3. `GET /v1/observe/debug` → player pos/zone/class/etc., no currency either.

## Expected
The API exposes the character's coin so an agent can budget purchases — e.g.
`/v1/observe/inventory` includes a `currency` object `{platinum, gold, silver, copper}` (and
ideally bank/cursor coin), or `/v1/observe/debug` includes the player's coin.
...
it to the `/v1/observe/inventory` (and/or `/v1/observe/debug`) JSON. The values already exist in
`game_state`; this is a plumbing/serialization addition, not new protocol work.

## Status
Fixed — `GET /v1/observe/inventory` and `GET /v1/observe/debug` now include a `currency` object
