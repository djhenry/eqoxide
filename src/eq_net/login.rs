    ...
    for attempt in 1..=max_retries {
        ...
        match run_login_phase(&config, &app_tx).await {
            ...
            Ok((stream, net_rx, gs, world_creds)) => {
                ...
                let navigator = Navigator::new(goto_target, entity_positions, entity_ids, zone_points, task_log, zone_cross, hail, say, target, attack, buy, move_req, give, collision, maps_dir);
