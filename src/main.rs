...
fn main() {
    ...
    let merchant_slots = ipc::MerchantSlots {
        ...
        open_await: Arc::new(Mutex::new(None)),
    };
    ...
    if !testzone_mode {
        ...
        let model_ctx = eqoxide::model::ModelContext {
            slots: eqoxide::eq_net::action_loop::ActionLoopSlots {
                nav:             nav_b,
                world:           world_b,
                quest:           quest_b,
            ...
            lifecycle: eqoxide::eq_net::gameplay::GameplayLifecycle {
                shutdown:            sd,
                camp:                cp,
                camp_until:          cu,
                respawn:             rsp,
    ...
    let mut application = eqoxide::app::App::new(
        ...
        camera,
        controller,
        eqoxide::app::NavHandles { shared_collision, zone_assets, nav_debug_view },
        eqoxide::app::SessionHandles {
            game_state_snapshot: game_state_snapshot.clone(),
            net_health: net_health_shared.clone(),
            acts: app_actions,
            spells: app_spells,
        },
        eqoxide::app::PublishedStateHandles {
            common_assets_failed,
            model_sync_dead,
            asset_sync_activity,
            frame_profile_shared: app_frame_profile,
            skin_cap_downgrades_shared: app_skin_cap_downgrades,
        },
        eqoxide::app::AssetServerConfig { asset_server_url, asset_user, asset_pass },
    );
