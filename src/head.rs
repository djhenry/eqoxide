...
//! High Elf (5), Dark Elf (6), Half Elf (7) and **female** Dwarf (8). Every other race's
//! painted Luclin hair renders the authored texels untinted, and classic `humhe*` heads
//! bake hair color into the face texture (the `haircolor` byte is never read there).
//!
...
/// painted Luclin hair (verified client-code trace, `hair-color.md` "Which code path
/// applies the tint"): High Elf (`HIE`), Dark Elf (`DKE`), Half Elf (`HEF`) — both
/// genders — and **female** Dwarf (`DWF`, gender 1). All other race/gender combos
/// (incl. HUM/BAR/ERU/ELF/HFL/GNM and male Dwarf) render their painted hair
/// untinted in the native client; tinting them was #519's raccoon-mask root cause.
/// `race` is the 3-letter code from `eq_race_to_code` (case-insensitive).
pub fn hair_tint_applies(race: &str, gender: u8) -> bool {
    ...
    matches!(&buf, b"HIE" | b"DKE" | b"HEF") || (&buf == b"DWF" && gender == 1)
}
...
mod tests {
    ...
    /// The native client's tint race gate (#519): only HIE/DKE/HEF (both genders) and
    /// FEMALE Dwarf are tinted. Everything else — most importantly HUM — must not be.
    #[test]
    fn hair_tint_gate_matches_native_client() {
        for g in [0u8, 1] {
            assert!(hair_tint_applies("HIE", g));
            assert!(hair_tint_applies("DKE", g));
            assert!(hair_tint_applies("HEF", g));
        }
